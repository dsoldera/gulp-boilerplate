var gulp = require('gulp'),
    ...
    jscs = require('gulp-jscs-with-reporter'),
    jscsReporter = require('gulp-jscs-html-reporter'),
...
gulp.task('js-validate', function () {
    ...
    return gulp
        ...
        .pipe(jscs.reporter(jscsReporter, {
            filename: config.paths.reportFolder + '/jscsReport.html'
        }))
        .pipe($.jshint())
...
gulp.task('scss-validate', function () {
    log('Analizing All SCSS Code of project with SASS-lint');

    var sassReportSonar = new sassReportSonarClass(config.paths.reportFolder + 'scssReport.json');
    sassReportSonar.openReporter(config.projectName, config.language);

    var stream = gulp.src(config.paths.styles.stylesValidate)
        .pipe($.sassLint({
...
gulp.task('sync', function () {
    ...
    log('Use this IP to access you project: ' + devip()[2] + ':3000');
